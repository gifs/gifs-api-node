...
const apiEndPointURL	   = util.format('%s/media', process.env.API_ENDPOINT_URL || 'https://api.gifs.com');
...
function upload(urlStr, formData, callback) {
  formData = util.isObject(formData) ? formData : {};
  if (!util.isObject(formData))
    return returnOrCallback(400, 'an object expected for "formData"', callback);
